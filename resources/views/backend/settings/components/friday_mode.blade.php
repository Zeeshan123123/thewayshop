		...
		<div class="grid grid-cols-4 gap-4 border-b border-gray-300 text-base">
			...
			<div class="lg:col-span-4 md:col-span-4 sm:col-span-4 col-span-4 bg-red">
	            <div class="my-2 text-sm">
	            	@php($label = 'Do you want  to enable black friday mode?')
                	@php($name = 'enable_friday_mode')
	                ...
	                <select name="{{ $name }}" type="text" autofocus id="{{ $name }}" value="{{ old($name) }}" class="rounded-sm focus:outline-none text-sm bg-gray-100 w-full" />
	                    ...
	                    <option value="yes" {{ "yes" == $setting->$name ? 'selected' : ''}}>Yes</option>
	                    <option value="no" {{ "no" == $setting->$name ? 'selected' : ''}}>No</option>
	                </select>
	            </div>

	            <button type="submit" class="md:w-60 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-8 md:px-12 text-xs md:text-base lg:text-base mb-4 rounded">{{ __('Update') }}</button>
	        </div>
...
<div>
	...
	<h1 class="w-full ml-16 mx-auto lg:text-xl md:text-base sm:text-sm text-sm font-bold mb-2 mt-6"> Black friday sale products </h1>

	<select x-cloak id="select" name="products[]" required="required">
		
		@foreach ( $products as $product )
			<option value="{{ $product->id }}">{{ $product->title }} | {{ $product->slug }}</option>
		@endforeach
	</select>

	<div x-data="dropdown()" x-init="loadOptions()" class="w-full ml-16 h-24 flex flex-col mx-auto">
		<form action="{{route('friday-mode.update')}}" method="POST">
			@csrf
		    ...
		    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-8 md:px-12 text-xs md:text-base lg:text-base mt-2 mb-4 rounded">Add Products</button>

		</form>

		@error('values')
		    <div class="red-text" style=" color: red; ">Please select products.</div>
		@enderror
	...
	@if( $products->count() > 0 )
		<div class="px-16 py-10 border-b border-gray-300 text-base">
			<h1 class="lg:text-xl md:text-base sm:text-sm text-sm font-bold mb-3"> My Products </h1>
			<ul>  
			@foreach ( $products as $product )
			    <li class="mt-1 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 text-xs mr-4">{{ $product->title }} <a href="{{-- route('personal-skills.remove', ['id' => $skill->id]) --}}" class="float-right cursor-pointer text-lg -mt-2" onclick="return confirm('Are you sure you want to delete this item?');"> x </a> </li>
			@endforeach
			</ul>
		</div>
	@endif

		...
		<div class="grid grid-cols-4 gap-4 border-b border-gray-300 text-base">
			...
			<div class="lg:col-span-4 md:col-span-4 sm:col-span-4 col-span-4 bg-red">
                <div class="my-2 text-sm">
                    ...
                    <input name="{{ $name }}" type="text" autofocus id="{{ $name }}" value="{{ $category->title }}" class="rounded-sm focus:outline-none text-sm bg-gray-100 w-full" placeholder="e.g. Graphics" />
			...
			<div class="lg:col-span-4 md:col-span-4 sm:col-span-4 col-span-4 bg-red">
	            <div class="my-2 text-sm">
	                ...
	                <select name="{{ $name }}" type="text" autofocus id="{{ $name }}" value="{{ old($name) }}" class="rounded-sm focus:outline-none text-sm bg-gray-100 w-full" />
	                    ...
	                    <option value="active" {{ "active" == $category->status ? 'selected' : ''}}>Active</option>
	                    <option value="inactive" {{ "inactive" == $category->status ? 'selected' : ''}}>Inactive</option>
	                </select>
	            </div>
	        </div>

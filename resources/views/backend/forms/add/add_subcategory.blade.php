	...
	<form action="{{ route('subcategory.store') }}" method="POST" enctype="multipart/form-data">
		...
		<div class="grid grid-cols-4 gap-4 border-b border-gray-300 text-base">
                ...
                <div class="my-2 text-sm">
                    ...
                    <select name="{{ $name }}" type="text" autofocus id="{{ $name }}" class="rounded-sm focus:outline-none text-sm bg-gray-100 w-full" />
                        ...
                        @foreach( $categories as $category )
                            <option value="{{$category->id}}" {{ $category->id == old($name) ? 'selected' : ''}}>{{$category->title}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
			...
			<div class="lg:col-span-4 md:col-span-4 sm:col-span-4 col-span-4 bg-red">
                <div class="my-2 text-sm">
                    ...
                    <input name="{{ $name }}" type="text" autofocus id="{{ $name }}" value="{{ old($name) }}" class="rounded-sm focus:outline-none text-sm bg-gray-100 w-full" placeholder="e.g. Beauty cream" />
		...
		<div class="lg:col-span-1 md:col-span-1 sm:col-span-2 col-span-2 bg-red">
            <div class="my-7 text-sm">
                <button type="submit" class="md:w-60 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-8 md:px-12 text-xs md:text-base lg:text-base mb-4 rounded">Store Sub Category</button>
            </div>
        </div>
